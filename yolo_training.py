...
class YOLOLoss(nn.Module):
    ...
    def box_giou(self, b1, b2):
        ...
        return giou
    def bbox_iou(box1, box2, x1y1x2y2=True, GIoU=False, DIoU=False, CIoU=False, MyIoU=True, eps=1e-7):
        ...
        # 目标框IOU损失函数的计算
        if CIoU or DIoU or GIoU or MyIoU:
            # 两个框的最小闭包区域的width
            cw = torch.max(b1_x2, b2_x2) - torch.min(b1_x1, b2_x1)
            # 两个框的最小闭包区域的height
            ch = torch.max(b1_y2, b2_y2) - torch.min(b1_y1, b2_y1)

            if CIoU or DIoU or MyIoU:
                # 最小外接矩形 对角线的长度平方
                c2 = cw ** 2 + ch ** 2 + eps  # 闭包矩形对角线长度的平方
                # 两个框中心点之间距离的平方
                ...
                # 将纵横比的损失项拆分成预测的宽高分别与最小外接框宽高的差值，加速了收敛提高了回归精度
                    ...
                    return iou - (rho2 / c2 + rho_w2 / cw2 + rho_h2 / ch2 + angle_cost)
